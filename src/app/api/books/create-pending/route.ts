...
import { ensureCustomerProfileForUser } from '@/lib/crm/service'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: book, error } = await adminSupabase
      ...
      .insert({
        user_id: user.id,
        title: getBookTitlePreview(params),
        status: 'draft',
    ...
    // Ensure customer profile exists so the Grow webhook can look up this user by email
    await ensureCustomerProfileForUser(user)

    return NextResponse.json({ bookId: book.id })
  } catch (error) {
    console.error('Create pending book error:', error)
